...
from scripts.utils import LoadJson, DataLoader, NNConverter
...
def find_layer_sizes(model_loc):
    ...
    layer_sizes_layers_dict = {
        "default": {"raw_batch": (4500000, -1), "encoded": (307200000, -1)},
        "32_32_32_32": {
...
np.savez(
    cwd + f"/data_for_latent/raw_dataset_{layer_sizes_name}.npz",
    data=torch_data_tensor,
    E=torch_E_tensor,
)  # Save File
...
np.savez(
    cwd + f"/data_for_latent/encoded_dataset_{layer_sizes_name}.npz",
    encoded_data,
)  # Save File to
