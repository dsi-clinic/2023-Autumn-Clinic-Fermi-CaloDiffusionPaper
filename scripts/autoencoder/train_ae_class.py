...
class AutoencoderTrainer:
    ...
    def _save_checkpoint(self, epoch: int, training_losses: np.ndarray, 
        ...
        """
        Save full training state after the training and evaluation 
        is complete for each epoch.
        """
        torch.save({
            'epoch': epoch,
    ...
    def _write_losses(self, loss_array: np.ndarray, filename:str) -> None:
        ...
        """
        with open(self.checkpoint_folder + "/" + filename + "_class.txt", "w") as fileout:
            fileout.write(
    ...
    def train(self) -> None:
        """
        Train the model for specified number of epochs. First loads training 
        ...
        if "train_loss_hist" in self.checkpoint.keys() and \
            not self.args.reset_training:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if not self.args.no_early_stop and \
                self.early_stopper.early_stop(val_loss - train_loss):
