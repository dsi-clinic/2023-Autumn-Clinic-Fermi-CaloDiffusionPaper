...
if __name__ == "__main__":
    ...
    def find_layer_sizes(model_loc):
        ...
        if layer_sizes_info.startswith("static"):
            layer_sizes = layer_sizes_info[
                len("static") + 1 : len("static") + 1 + 11
            ]
        else:
            layer_sizes = "default"

        layer_sizes_enum_dict = {
            "default": "default",
            "32_32_32_32": "4x32",
            "16_16_16_16": "4x16",
        ...
        layer_sizes_layers_dict = {
            "default": {"raw_batch": (4500000, -1), "encoded": (3200000, -1)},
            "32_32_32_32": {
                "raw_batch": (44528000, -1),
                "encoded": (108416000, -1),
            },
            "16_16_16_16": {
                "raw_batch": (4500000, -1),
        ...
        dict_info = [
            layer_sizes_enum_dict[layer_sizes],
            layer_sizes_layers_dict[layer_sizes],
        ]
    ...
    parser.add_argument(
        "--data_folder",
        default="/net/projects/fermi-1/doug/2023-Autumn-Clinic-Fermi-CaloDiffusionPaper/data/dataset_2/",
        help="Folder containing data and MC files",
    )
    parser.add_argument(
        "-c",
        "--config",
        default="/net/projects/fermi-1/doug/2023-Autumn-Clinic-Fermi-CaloDiffusionPaper/configs/config_dataset2.json",
    ...
    parser.add_argument(
        "--binning_file",
        type=str,
        default="/net/projects/fermi-1/doug/2023-Autumn-Clinic-Fermi-CaloDiffusionPaper/CaloChallenge/code/binning_dataset_2.xml",
    )
    parser.add_argument(
        "--model_loc",
        default="/net/projects/fermi-1/doug/ae_models/dataset2_AE/downsample_2_8_hrs/final.pth",
        help="Location of model",
    )

    flags = parser.parse_args()

    layer_sizes_name, layer_sizes_layers, layer_sizes = find_layer_sizes(
        flags.model_loc
    )
    ...
    figure_directory = (
        "./autoencoder/pca_plots/victor_plots/"
        + month_day
        + dt_string
        + f"-{layer_sizes_name}/"
    )
    ...
    pull_path = (
        f"./../data_for_latent/raw_dataset_victor1_{layer_sizes_name}.npz"
    )
    pull_path2 = (
        f"./../data_for_latent/encoded_dataset_victor1_{layer_sizes_name}.npz"
    )
    ...
    raw_sizes, encoded_sizes = (
        layer_sizes_layers["raw_batch"],
        layer_sizes_layers["encoded"],
    )

    print(f"raw sizes: {raw_sizes}")
    print(f"encoded sizes: {encoded_sizes}")

    batch_reshape = raw_data.reshape(
        len(raw_data), -1
    )  # (100000, 6480) #4500000
    batch_reshape_stack = raw_data.reshape(
        raw_sizes[0], raw_sizes[1]
    )  # 4500000,
    ...
    sample_raw_data = batch_reshape[
        np.random.choice(batch_reshape.shape[0], 5000, replace=False)
    ]
    sample_encoded_data = encoded_data[
        np.random.choice(encoded_data.shape[0], 5000, replace=False)
    ]

    # sample_raw_data_radial = batch_reshape_radial[np.random.choice(batch_reshape_radial.shape[0], 5000, replace=False)]
    sample_raw_data_stack = batch_reshape_stack[
        np.random.choice(batch_reshape_stack.shape[0], 5000, replace=False)
    ]
    ...
    print(
        "Number of principal components to explain 95 percent of data (RAW)=",
        len(features),
    )
    plt.figure()
    ...
    features = range(len(pca.explained_variance_))
    print(
        "Number of principal components to explain 95 percent of data (Radial Raw) =",
        len(features),
    )
    plt.figure()
    total_variance = np.sum(pca.explained_variance_)
    ...
    features = range(len(pca.explained_variance_))
    print(
        "Number of principal components to explain 95 percent of data (Encoded)=",
        len(features),
    )
    plt.figure()
    total_variance = np.sum(pca.explained_variance_)
