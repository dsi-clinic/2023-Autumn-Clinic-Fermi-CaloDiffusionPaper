...
from utils import create_R_Z_image, create_phi_image
from models import cosine_beta_schedule, FCN, CondUnet, extract
...
class CaloDiffu(nn.Module):
    ...
    def __init__(
        ...
        if self.fully_connected:
            ...
            self.model = CondUnet(
                ...
                out_dim=out_dim,
                channels=in_channels,
                layer_sizes=[in_channels] * len(layer_sizes),
                block_attn=block_attn,
                mid_attn=mid_attn,
                cylindrical=config.get("CYLINDRICAL", False),
                compress_Z=compress_Z,
                data_shape=summary_shape,
                cond_embed=(self.E_embed == "sin"),
                time_embed=(self.time_embed == "sin"),
                max_downsample=max_downsample,
                is_latent=self.is_latent,
    ...
    def noise_image(self, data=None, t=None, noise=None):
        ...
        if self.discrete_time:
            sqrt_alphas_cumprod_t = extract(
                self.sqrt_alphas_cumprod, t, data.shape
            )
            sqrt_one_minus_alphas_cumprod_t = extract(
                self.sqrt_one_minus_alphas_cumprod, t, data.shape
            )
            out = (
                sqrt_alphas_cumprod_t * data
    ...
    def compute_loss(
        ...
        if self.discrete_time:
            ...
            x_noisy = (
                data + torch.reshape(sigma, (data.shape[0], 1, 1, 1, 1)) * noise
            )
            sigma2 = sigma**2
        ...
        if "hybrid" in self.training_obj:
            ...
            c_skip = torch.reshape(
                1.0 / (sigma2 + 1.0), (data.shape[0], 1, 1, 1, 1)
            )
            c_out = torch.reshape(
                1.0 / (1.0 + 1.0 / sigma2).sqrt(), (data.shape[0], 1, 1, 1, 1)
            )
            weight = torch.reshape(
                1.0 + (1.0 / sigma2), (data.shape[0], 1, 1, 1, 1)
            )
            ...
            if "energy" in self.training_obj:
                ...
                x0_pred = (
                    x_noisy - sqrt_one_minus_alphas_cumprod_t * pred
        ...
        if loss_type == "l1":
            ...
            if "weight" in self.training_obj:
                loss = (weight * ((pred - data) ** 2)).sum() / (
                    torch.mean(weight) * self.nvoxels
                )
            else:
    ...
    def denoise(self, x, E, t_emb):
        ...
        if "mean_pred" in self.training_obj:
            ...
            c_out = torch.sqrt(sigma2) / (sigma2 + 1.0).sqrt()
    ...
    def p_sample(
        ...
        if "noise_pred" in self.training_obj:
            ...
            x0_pred = pred
            noise_pred = (
                x - sqrt_alphas_cumprod_t * x0_pred
            ) / sqrt_one_minus_alphas_cumprod_t
        elif "hybrid" in self.training_obj:

            sigma2 = (
                extract(self.sqrt_one_minus_alphas_cumprod, t, x.shape) ** 2
            )
            c_skip = 1.0 / (sigma2 + 1.0)
            c_out = torch.sqrt(sigma2) / (sigma2 + 1.0).sqrt()
        ...
        if debug:
            if x0_pred is None:
                x0_pred = (
                    x - sqrt_one_minus_alphas_cumprod_t * noise_pred
                ) / sqrt_alphas_cumprod_t
            return out, x0_pred
        return out
    ...
    def Sample(
        ...
        # avg_shower = std_shower = None (FOR PYFLAKES)
        ...
        for time_step in time_steps:
            times = torch.full(
                (gen_size,), time_step, device=device, dtype=torch.long
            )
